...
#            entry = json.loads(ln[:-1].decode('utf-8'))
    ...
    def shortest_path_decoding(self, sentence_lemmas, start=0, in_gap=False, max_gap_length=None):
        ...
        #sentence_lemmas = [str(l, 'utf-8') for l in sentence_lemmas]
        queue = []
        ...
        while True:
            ...
            #path = unwrap(list(map(list, path)))

            #ANDREJ DEBUG
            heap_addition = (len(path)+1, e-1, e, [[l]]+path, ('o' if in_gap else 'O')+tags, [[e-1,('o' if in_gap else 'O'),[e-1,],False,None]]+tokinfo)
            heappush(queue, heap_addition)
            #+e
            for cand in self.signatures_by_last_lemma(l):
                ...
                if sentence_lemmas[b:e]==candL:
                    ...
                    #path = unwrap(list(map(list, path)))
                    #heap_addition = (len(path)+1, b, e, [cand]+path, newtags+tags, newtokinfo+tokinfo)
                    heap_addition = (len(path)+1, b, e, list(map(list,cand))+path, newtags+tags, newtokinfo+tokinfo)
                    heappush(queue, heap_addition)
                    #+b+1
                    ...
                    if subspans:
                        ...
                        for before_gap,after_gap in zip(subspans[1:],subspans[:-1]):
                            ...
                            newpath = list(map(list, gpath)) + newpath
                        ...
                        #ANDREJ DEBUG
                        newpath = list(map(list, cand)) + newpath
                        ...
                        #path = unwrap(list(map(list, path)))
                        heap_addition = (len(path)+1+gapCost, b, e, newpath+path, newtags+tags, newtokinfo+tokinfo)
                        heappush(queue, heap_addition)
                        #+b+1
            if not queue:
